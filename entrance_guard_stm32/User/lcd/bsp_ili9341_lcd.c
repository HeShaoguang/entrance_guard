...
void ILI9341_DispNum ( uint16_t x, uint16_t y, uint32_t num, uint8_t len ,uint8_t size)
{
	uint8_t t,temp;
	uint8_t enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				ILI9341_DispChar_EN(x+(size/2)*t,y,' ');
				continue;
			}else enshow=1; 
		 	 
		}
	 	ILI9341_DispChar_EN(x+(size/2)*t,y,temp+'0'); 
	}
}
/**
