...
//要修改就修改XPT2046_TouchDown（）函数和XPT2046_TouchUp（）函数
//其他东西都不用碰
//XPT2046_TouchDetect（）这个函数很关键 要理解好！
...
/** 第0层封装.软件模拟spi写一个字节，发送0xD0或者0x90返回触摸的返回值。第一层封装为XPT2046_ReadADC()
  * @brief  XPT2046 的写入命令
  * @param  ucCmd ：命令
  *   该参数为以下值之一：
  *     @arg 0x90 :通道Y+的选择控制字
...
/**第0层封装.软件模拟spi读一个返回值，因为adc返回值是一个12位的值，所以返回一个16位
  * @brief  XPT2046 的读取命令
  * @param  无
  * @retval 读取到的数据
  */
...
/** 3.第三次封装，获得xy坐标，只不过是经过了过滤，和放到一个数据里
...
static uint8_t XPT2046_ReadAdc_Smooth_XY ( strType_XPT2046_Coordinate * pScreenCoordinate )
{
	...
	int16_t sAD_X, sAD_Y;
	int16_t sBufferArray [ 2 ] [ 9 ] = { { 0 }, { 0 } };  //坐标X和Y进行9次采样，并保存在这个数组里
...
/**
  ...
  * @param  pTouchPara：坐标校准系数，校准之后的数组
  * @retval 获取情况
	*   该返回值为以下值之一：
  *     @arg 1 :获取成功
  *     @arg 0 :获取失败
